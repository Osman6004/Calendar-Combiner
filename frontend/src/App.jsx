...
function getOrCreateSessionId() {
  ...
  if (!sid) { sid = crypto.randomUUID(); localStorage.setItem('calendar_session_id', sid) }
  return sid
}
const SESSION_ID = getOrCreateSessionId()

function Avatar({ name, index }) {
  const colors = [{ bg: '#dbeafe', text: '#1d4ed8' }, { bg: '#dcfce7', text: '#15803d' }, { bg: '#fef3c7', text: '#b45309' }]
  const c = colors[index % colors.length]
  const initials = name.split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase()
  return <div style={{ width: 38, height: 38, borderRadius: '50%', background: c.bg, color: c.text, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 500, fontSize: 13, flexShrink: 0 }}>{initials}</div>
}
...
export default function App() {
  ...
  useEffect(() => {
    const today = new Date(); const next = new Date()
    ...
    <div className="page">
      <div className="card">
        ...
        <section>
          ...
          <div className="participants">
            {participantArr.map(({ index, data }) => (
              <div className="participant-row" key={index}>
                {data ? <Avatar name={data.name} index={index} /> : <div className="avatar-placeholder">{index + 1}</div>}
          ...
          {participantCount < 3 && <button className="btn-add" onClick={() => setParticipantCount(3)}>+ Add a third person</button>}
        </section>
        <section>
        ...
        <section>
          ...
          <div className="row-2">
            <div className="field"><label>Start</label><input type="time" value={timeFrom} onChange={e => setTimeFrom(e.target.value)} /></div>
            <div className="field"><label>End</label><input type="time" value={timeTo} onChange={e => setTimeTo(e.target.value)} /></div>
          </div>
        </section>
        <section>
          ...
          <div className="row-2">
            {[15, 30, 45, 60].map(d => (
              <button key={d} className={`btn-pill ${duration === d ? 'active' : ''}`} onClick={() => setDuration(d)}>{d} min</button>
            ))}
          </div>
        </section>
        {error && <div className="error-msg">{error}</div>}
        <div className="actions">
          <button className="btn-primary" onClick={findOverlaps} disabled={loading}>{loading ? 'Finding...' : 'Find overlapping times'}</button>
          <button className="btn-secondary" onClick={() => setShareVisible(v => !v)}>Share session link</button>
        </div>
        {shareVisible && (
          <div className="share-box">
            <p>Share this link so others can connect their calendar to this session:</p>
            <div className="share-link-row">
              <code>{getShareLink()}</code>
        ...
        {slots !== null && (
          <section>
            <div className="section-label" style={{ marginBottom: 12 }}>Overlapping free times — {slots.length} slot{slots.length !== 1 ? 's' : ''} found</div>
            {slots.length === 0
              ? <p className="empty-msg">No overlapping free times found. Try a wider date range or different hours.</p>
              : <div className="slots-list">
                  {slots.slice(0, 25).map((slot, i) => {
                    ...
                    const startT = start.toLocaleTimeString('en-CA', { hour: '2-digit', minute: '2-digit' })
                    const endT = end.toLocaleTimeString('en-CA', { hour: '2-digit', minute: '2-digit' })
                    return (
                      <div className="slot-row" key={i}>
                        <span className="slot-day">{day}</span>
                        <span className="slot-time">{startT} – {endT}</span>
                        <span className="slot-dur">{slot.duration_minutes} min</span>
                      </div>
                    )
                  })}
                  {slots.length > 25 && <p className="more-msg">+{slots.length - 25} more slots</p>}
                </div>
            }
            {slots.length > 0 && <button className="btn-lettuce" onClick={openLettuceMeet}>Open in LettuceMeet →</button>}
          </section>
        )}
      </div>
    </div>
